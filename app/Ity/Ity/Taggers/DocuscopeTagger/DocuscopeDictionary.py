    ...
    with openAny(dictionaryDirectory, ["_tones.txt", "tones.txt"]) as f:
        for r in f:
            ...
            if len(l) > 1:
                ...
                elif l[0] == "LAT:" or l[0] == "LAT*:" or l[0] == "CLASS:":
...
class DocuscopeDictionary:
    def __init__(self, dictname="default", debug=False):
        ...
        self.default_directory = findDict("default")
        self.directory = findDict(dictname)
        self.debug = debug
        self.ruleCount = 0

        # self.logger.info("DocuscopeDictionary.__init__(): self.dictionary = {}".format(self.directory))
        ...
        # August 7, 2017 - Changed 'self.directory' to 'self.defulat_directory'.
        # _tones.txt is from the custom dictionary directory. But, all the other files
        # are from the default folder.
        with openAny(self.default_directory, ["_wordclasses.txt", "wordclasses.txt"]) as f:
            curClass = "NONE"
            for r in f:
                l = r.split()
                if len(l) == 1:
        ...
        for lat in self.lats:                           # for each LAT
            for latfs in self.lats[lat][3]:             # for each LAT files
                # self.logger.info("latfs = {}".format(latfs))
                try:
                    with open(makePath(self.default_directory, latfs+".txt"), errors="ignore") as f:
                        for r in f:
                            li = re.findall(r"[!?\w'-]+|[" + string.punctuation + "]", r)
                            # li = [i for i in WordBreaker(r)]
                            l = list(map(lambda x: x.upper() if x[0]=='!' else x.lower(), li))
...
# directory
dictionarySearchPath = ["..", "../Docuscope", ".", "docuscope/Ity/Data/Dictionaries/Docuscope"]

def findDict(dictname=None):
    if isdir(dictname):
        return dictname
    else:
        for d in dictionarySearchPath:
            path = makePath(d, dictname)
            if isdir(path):
                return path
    raise IOError(999, "Can't find Dictionary: {}. Working dir: {}".format(path, os.getcwd()))
...
def openAny(dir, fnames):
    ...
    for f in fnames:
        try:
            return open(makePath(dir, f), "r", errors="ignore")
        except IOError:
            pass
    raise IOError
...
def getDict(dictn=None, force=False):
