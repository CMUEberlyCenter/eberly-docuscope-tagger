...
class DocuscopeTaggerNeo(Tagger):
    ...
    def _long_rule_applies_at_token_index(self, rule: tuple[str]):
...
SEPARATOR = f"{hash('+++Michael Ringenberg, DocuScope, CMU, a separator+++')}"
def get_lat_rules(
        trx: Transaction,
        first_tokens: tuple[str],
        second_tokens: tuple[str],
        third_tokens: tuple[str]) -> list[Rule]:
    """ Retrieve the LAT rules starting with the given bi-/tri-gram. """
    hsh = ",".join([*first_tokens, SEPARATOR, *second_tokens, SEPARATOR, *third_tokens])
    res = CACHE.get(hsh)
    if res == None:
        result = trx.run(
            "MATCH p = (s:Start)-[n:NEXT]->()-[m:NEXT]->()-[*0..25]->(l:Lat) "
...
class LRUCache:
    ...
    def put(self, key: str, value: list[Rule]) -> None:
